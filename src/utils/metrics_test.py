...
class TestComputeHuffmanNominalComplexity(unittest.TestCase):
    ...
    def test_all_identical(self):
        ...
        self.assertAlmostEqual(
            compute_huffman_nominal_complexity(q),
            expected,
            places=6
        )
    ...
    def test_three_to_one_ratio(self):
        ...
        entropy = -(0.75 * np.log2(0.75) + 0.25 * np.log2(0.25))  # ~0.811278 bits
...
def apply_alpha_dict(qmodel, alpha_dict):
    ...
    colo/model_evalution in QTensor/src/examples/functions.py. When merged,
    import that functions insted of redefining it here. """
...
class TestLeNetQuantizedComplexity(unittest.TestCase):
    def setUp(self):
        ...
        self.model = models.Sequential([
            layers.Conv2D(6, kernel_size=5, activation='relu', padding='same', name="conv2d"),
            layers.AveragePooling2D(name="pool1"),
            layers.Conv2D(16, kernel_size=5, activation='relu', name="conv2d_1"),
    ...
    def test_uniform_quantizer_space_complexity(self):
        ...
        for i, layer in enumerate(layer_names):
            ...
            qconfig[layer]["weights"]['kernel'] = UniformQuantizer(bits=kernel_bits[i], signed=True)
            qconfig[layer]["weights"]['bias']   = UniformQuantizer(bits=bias_bits[i]  , signed=True)
        #for key in qconfig:
        #    print(f'{key}: {qconfig[key]}')

        # 4) compute expected size
        expected_bits = 0
        for kb, ks, bb, bs in zip(kernel_bits, self.kernel_size, bias_bits, self.bias_size):
        ...
        alpha_dict = {
            ...
            for layer_name, kalpha, balpha in zip(qconfig, kernel_alphas, bias_alphas)
        }
    ...
    def test_flex_quantizer_space_complexity(self):
        ...
        def random_probability_vector(n, epsilon=1e-8):
            vec = np.random.rand(n) + epsilon  # ensure all elements are strictly > 0
            return vec / vec.sum()
        ...
        for i, layer in enumerate(layer_names):
            qconfig[layer] = dict()
            qconfig[layer]['weights'] = dict()
            qconfig[layer]["weights"]['kernel'] = FlexQuantizer(bits=kernel_bits[i], n_levels=kernel_n_levels[i], signed=True)
            qconfig[layer]["weights"]['bias']   = FlexQuantizer(bits=bias_bits[i]  , n_levels=bias_n_levels[i]  , signed=True)
        #for key in qconfig:
        #    print(f'{key}: {qconfig[key]}')
        ...
        groups = [
            (self.kernel_shape, self.kernel_size, kernel_bits,   kernel_n_levels,   kernel_alphas,   kernel_probabilities, kernels),
            (self.bias_shape,   self.bias_size,   bias_bits,     bias_n_levels,     bias_alphas,     bias_probabilities,   biases),
        ]
        for shape_list, size_list, bits_list, levels_list, alphas_list, probs_list, container in groups:
            for shape, size, bits, n_levels, alpha, probs in zip(
                shape_list, size_list, bits_list, levels_list, alphas_list, probs_list
            ):
                ...
                values = np.random.choice(valid_values, size=n_levels, replace=False)
                vector = np.random.choice(values, size=size, replace=True, p=probs)
                ...
                assert np.all(mask), f"These values are not valid: {weight[~mask]}"

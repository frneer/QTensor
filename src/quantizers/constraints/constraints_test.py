...
import unittest
from constraints import PositiveConstraint, LevelConstraint, ThresholdConstraint
...
class TestLevelConstraint(unittest.TestCase):
    def test_clipping(self):
        ...
        assertListAlmostEqual(self, lc(tf.constant([-2, -1, 0, 1, 2], dtype=tf.float32)).numpy().tolist(), [-1.0, -1.0, 0.0, 3/4, 3/4])

    def test_ordering(self):
        alpha = tf.Variable(100.0)
        lc = LevelConstraint(alpha, m_levels=8, signed=True)

        assertListAlmostEqual(self, lc(tf.constant([2, 1, 0, -1, -2], dtype=tf.float32)).numpy().tolist(), [-100, -1, 0, 1, 2])
    ...
    def test_update_alpha(self):
        ...
        alpha.assign(2.0)
        assertListAlmostEqual(self, lc(tf.constant([-2, -1, 0, 1, 2], dtype=tf.float32)).numpy().tolist(), [-2.0, -1.0, 0.0, 1.0, 1.5])
...
class TestThresholdConstraint(unittest.TestCase):
    def test_clipping(self):
        ...
        assertListAlmostEqual(self, tc(tf.constant([-2, -1, 0, 1, 2], dtype=tf.float32)).numpy().tolist(), [-1.0, -1.0, 0.0, 1.0, 1.0])

    def test_ordering(self):
        alpha = 100.0
        tc = ThresholdConstraint(alpha)

        assertListAlmostEqual(self, tc(tf.constant([2, 1, 0, -1, -2], dtype=tf.float32)).numpy().tolist(), [-100, -1, 0, 1, 100])
    ...
    def test_update_alpha(self):
        ...
        alpha.assign(2.0)
        assertListAlmostEqual(self, tc(tf.constant([-2, -1, 0, 1, 2], dtype=tf.float32)).numpy().tolist(), [-2.0, -1.0, 0.0, 1.0, 2.0])

...
qconfig_flex = {
        "conv2d"    : { "weights": {"kernel": FlexQuantizer(bits=8, n_levels=30, signed=True)}, "activations": {"activation": UniformQuantizer(bits=8 , signed=False)}, },
        "conv2d_1"  : { "weights": {"kernel": FlexQuantizer(bits=8, n_levels=10, signed=True)}, "activations": {"activation": UniformQuantizer(bits=8 , signed=False)}, },
        "dense"     : { "weights": {"kernel": FlexQuantizer(bits=8, n_levels=10, signed=True)}, "activations": {"activation": UniformQuantizer(bits=8 , signed=False)}, },
        "dense_1"   : { "weights": {"kernel": FlexQuantizer(bits=8, n_levels=20, signed=True)}, "activations": {"activation": UniformQuantizer(bits=8 , signed=False)}, },
        #"dense_2"   : { "weights": {"kernel": FlexQuantizer(bits=8, n_levels=5, signed=True)}, "activations": {"activation": UniformQuantizer(bits=8 , signed=False)}, },
        }

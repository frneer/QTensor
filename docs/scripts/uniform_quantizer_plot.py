...
def main():
    ...
    plt.step(input_levels, levels, label=r'$q\left(x; \alpha\right)$', color='blue', linestyle='-', lw=1)
    ...
    if plot_ste:
        ...
        ## x < -alpha
        plt.plot(dq_dx_neg, -np.ones(len(dq_dx_neg)), color='purple', linestyle='-', lw=2, alpha=0.5)
        ## x > alpha
        plt.plot(dq_dx_pos, 3/4 * np.ones(len(dq_dx_pos)), label=r'STE', color='purple', linestyle='-', lw=2, alpha=0.5)
    ...
    ## Set major ticks
    labels=[r'$-\alpha$', r'$-\frac{\alpha}{2}$', '0', r'$\frac{\alpha}{2}$', r'$\alpha$']

...
\begin{itemize}
    \item En el caso signado, el rango es $r(\alpha) = 2\alpha$, definido por el intervalo $[-\alpha, \alpha].$
    \item En el caso no signado, el rango es $r(\alpha) = \alpha$, definido por el intervalo $[0, \alpha].$
\end{itemize}
...
% \TODONT{Ver si hay que desarrollar xq es malo para punto fijo}
...
donde $q_{u}$ es la función de cuantización uniforme definida \eqref{eq:uniform_quantizer}.

El gráfico de la función de cuantización flexible se muestra en la \reffig{flexible_signed_q}, que muestra en amarillo, los niveles previos a la aplicación del cuantizador uniforme y, en azul, los niveles finales tras aplicar dicho cuantizador.

...
Un desafío intrínseco de desconocer la naturaleza de la distribución de los datos, y de depender de un conjunto finito de entrada y salida, es obtener un modelo que sea capaz de generalizar la relación aprendida a datos no vistos previamente. Además, el hecho de que el conjunto de datos contenga ruido e incertidumbre, puede llevar a que el modelo se ajuste demasiado a las particularidades del conjunto de entrenamiento, y pierda la capacidad de generalización. Este fenómeno es conocido como sobreajuste (\textit{overfitting} en inglés) \cite{Bishop2006}.
...
Como se mencionó anteriormente, la función de costo $\mathcal{L}$ raramente tiene un único término, el componente principal suele ser el término de pérdida $\mathcal{L}_{l}$, que mide la discrepancia entre las predicciones del modelo ($\hat{y}$) y los valores reales ($y$) del conjunto de entrenamiento. Sin embargo, también se desea ponderar otros aspectos. Por ello, es muy común añadir términos de regularización a la función de costo $\mathcal{L}_{r}$, que penalizan ciertas características del modelo, como la complejidad o la magnitud de los parámetros \cite{Goodfellow2016}. La forma típica de la función de costo se expresa como \eqref{eq:cost_function}.

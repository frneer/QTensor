...
El problema fundamental que se busca resolver en el modelado de sistemas: encontrar los parámetros óptimos $\theta$ que minimicen la función de costo $\mathcal{L}$. Este proceso suele denominarse entrenamiento del modelo.
...
Como se mencionó anteriormente, la función de costo $\mathcal{L}$ raramente tiene un único término, el componente principal suele ser el término de pérdida $\mathcal{L}_{l}$, que mide la discrepancia entre las predicciones del modelo ($\hat{y}$) y los valores reales ($y$) del conjunto de entrenamiento. Sin embargo, como se mencionó, también se desea ponderar otros aspectos. Por ello, es muy común añadir términos de regularización a la función de costo $\mathcal{L}_{r}$, que penalizan ciertas características del modelo, como la complejidad o la magnitud de los parámetros \cite{Goodfellow2016}. La forma típica de la función de costo se expresa como en la \refeq{cost_function}.

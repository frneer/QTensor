...
Este principio se formaliza mediante la función de verosimilitud $V(\theta)$, que mide la probabilidad conjunta de observar todos los datos de la muestra (de tamaño $n$) dados los parámetros $\theta$, y se expresa como el producto de las probabilidades individuales \eqref{eq:likelihood} \cite{Bishop2006}.
...
Por conveniencia computacional, se trabaja con el logaritmo de la función de verosimilitud, conocido como log-ve\-ro\-si\-mi\-li\-tud, que convierte el producto en una suma, tal como se muestra \eqref{eq:log_likelihood}.
...
En consecuencia, la función de costo se define como el negativo de la log-ve\-ro\-si\-mi\-li\-tud; por lo tanto, el problema se plantea como la minimización de dicha función (equivalente a maximizar la log-ve\-ro\-si\-mi\-li\-tud) con respecto a los parámetros $\theta$.

En este problema, no existe una solución cerrada para optimizar la función de costo, por lo que se deben emplear métodos numéricos iterativos. Entre estos métodos, destacan los algoritmos de optimización basados en la utilización del gradiente de la función de costo \cite{Nocedal2006}, estos serán detallados en la sección \ref{sec:algoritmos_de_optimizacion}.
...
% \TODONT{Revisar esta ultima parte}

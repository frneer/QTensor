...
El modelo de regresión logística se utiliza en problemas de clasificación binaria. Estima la probabilidad de que una observación pertenezca a una clase mediante la función sigmoide \cite{Bishop2006}, como se muestra en la \refeq{logistic_regression}.
...
Por conveniencia computacional, se trabaja con el logaritmo de la función de verosimilitud, conocido como log-verosimilitud, que convierte el producto en una suma, como se muestra en la \refeq{log_likelihood}.
...
Estos algoritmos se pueden clasificar principalmente por la cantidad de información que utilizan. La familia más común son los métodos de primer orden, como el algoritmo de gradiente descendente (GD). Estos consisten en actualizar en cada iteración los parámetros moviéndose en la dirección opuesta al gradiente (la dirección de máximo decrecimiento) de la función de costo. Este fue, de hecho, el método utilizado para ajustar los parámetros en el modelo que genera la \reffig{logistic_regression}.
...
Por esta razón, los métodos de primer orden como el algoritmo de gradiente descendente son los utilizados en el entrenamiento de redes neuronales.

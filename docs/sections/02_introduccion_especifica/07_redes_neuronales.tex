...
La predicción $\hat{\mathbf{y}}$, para una entrada $\mathbf{x}$, se genera propagando la información a través de múltiples capas, desde la entrada hasta la salida. Por ende, la expresion matematica es una composición de muchas funciones no lineales (véase \refeq{mlp}). Si bien la solucion analitica de los gradientes es sencilla de describir, los computos de estos valores durante el entrenamiento son computacionalmente complejos.

Para abordar este problema se emplea el algoritmo de retropropagación (\textit{backpropagation}) \cite{rumelhart1986learning}. Este procedimiento aplica la regla de la cadena de manera recursiva para descomponer el gradiente total en los gradientes de cada capa: primero se calcula el error en la salida y luego se propaga hacia atrás a través de la red, multiplicando. Este algoritmo permite obtener de forma eficiente los gradientes necesarios para actualizar los parámetros en cada iteración. \textit{backpropagation} es simplemente un caso particular de un grupo de algortimos cuyo objetivo es el calculo de derivadas de manera automática, llamada diferenciación automática, generalmente conocido como \textit{autodiff} (abreviado del inglés \textit{automatic differentiation}), la implementación computacional de este algoritmo explicará en la sección \ref{sec:autodiff} \cite{Goodfellow2016}.
...
En resumen, una iteración o época del algoritmo de entrenamiento de una red neuronal consta de los siguientes pasos:

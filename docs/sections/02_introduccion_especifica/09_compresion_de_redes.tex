...
En el contexto de la cuantización de redes neuronales es necesaria una alta precisión numérica durante el entrenamiento de las mismas por lo que este proceso se realiza generalmente en punto flotante de 32 bits (FP32), como fue explicado en la sección \ref{sec:motivacion}. Luego, al aplicar cuantizaciones se suelen utilizar representaciones en punto fijo, las cuales suelen ser implementadas con aritmetica entera en hardware (INT8, INT4).
...
Como se explicó, en la sección \ref{sec:entrenamiento_redes_neuronales}, el proceso de entrenamiento de una red neuronal requiere de una alta precisión numérica, y suele realizarse en FP32, siendo particularmente crítico para el \textit{backward pass}, al depender de cálculos precisos para actualizar los pesos de manera efectiva y asegurar la convergencia del entrenamiento.
...
En este trabajo se utilizarán STE y PW-STE para aproximar las funciones de cuantización durante el entrenamiento con QAT, por su simplicidad y en trabajos futuros se explorarán otros estimadores de las derivadas y las mejoras que puedan aportar durante el entrenamiento.

Asimismo, como se describio en la sección \ref{sec:objetivos}, se tiene como objetivo implentar cuantizadores con parámetros entrenables. Para esto, es necesario tomar el gradiente de la función de cuantización con respecto a sus parámetros y actualizar estos parámetros durante el proceso de entrenamiento utilizando el mismo algoritmo de optimización utilizado para los pesos de la red neuronal explicado en esta sección.

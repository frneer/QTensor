...
La salida de la capa se representa como en \eqref{eq:single_layer_forward} y el gradiente con respecto a los pesos $\theta$ como en \eqref{eq:single_layer_backward} Y, de manera similar, para propagar el gradiente hacia la capa anterior ($m-1$) se tiene \refeq{single_layer_backward_activation}.
...
La salida de la capa queda definida formalmente en la \refeq{single_layer_forward_quantizer}, donde $z$ ahora tiene como entrada los pesos cuantizados $q_{\theta}(\theta)$ y la activación de salida está cuantizada mediante $q_{\phi}()$. Los gradientes de estas cuantizaciones son $\nabla_{q_{\theta}} = \frac{\partial q_{\theta}}{\partial \theta} = q_{\theta}'$ y $\nabla_{q_{\phi}} = \frac{\partial q_{\phi}}{\partial \phi} = q_{\phi}'$.
...
Por lo tanto, las funciones de cuantización son parte también del camino del gradiente, la expresión de la derivada de la función de costo con respecto a $\theta$ es \refeq{single_layer_backward_quantizer}.

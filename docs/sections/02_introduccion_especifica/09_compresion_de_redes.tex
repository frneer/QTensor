...
En este caso los umbrales de cuantización quedan definidos por \eqref{quantization_thresholds_example}.
\begin{equation}
...
Las figuras \ref{fig:training} y \ref{fig:qat} muestran, a manera ilustrativa, las diferencias entre el proceso de entrenamiento tradicional y el proceso de entrenamiento con cuantización (QAT). Sin embargo, la \reffig{qat} esconde un detalle fundamental.
...
\begin{equation}
    ...
    \frac{\partial \mathcal{L}}{\partial \theta} =
    \underbrace{\overline{y_{m}\,}\dfrac{\partial z}{\partial \theta}\,\phi'}_{\text{\tiny Equivalente a la expresión \eqref{eq:single_layer_backward}}}\!\cdot\!\underbrace{q_{\phi}'\,q_{\theta}'}_{\text{\tiny Términos de cuantización}}
\end{equation}
...
En este trabajo se tiene como objetivo implentar cuantizadores con parámetros entrenables. Para esto, simplemente es necesario tomar el gradiente de la función de cuantización con respecto a sus parámetros y actualizar estos parámetros durante el proceso de entrenamiento utilizando el mismo algoritmo de optimización utilizado para los pesos de la red neuronal explicado en esta sección.
...
En resumen, el proceso de entrenamiento con QAT con parámetros entrenables consiste en realizar el re-entrenamiento del modelo, mediante el entrenamiento con efectos de cuantización, con aproximaciones de las funciones de cuantización para evitar el desvanecimiento del gradiente. Este proceso que ajusta los pesos del modelo y de sus cuantizadores de manera conjunta se ilustra en la \reffig{qat_process}.

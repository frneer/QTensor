...
El entrenamiento del modelo consiste en definir el conjunto de datos de entrenamiento \texttt{(x\_train, y\_train)}, definir la función de costo (entropía cruzada \cite{Goodfellow2016}), el algoritmo de optimización (en este caso ADAM), explicado en la sección \ref{sec:entrenamiento_redes_neuronales}, y la métrica de evaluación (precisión), descrita en la sección \ref{sec:metricas}. Finalmente, se llama a la función \texttt{fit} que entrena el modelo con los datos de entrenamiento, tal como se muestra en el fragmento de código \ref{code:tensorflow_mlp_train}.
...
Esta biblioteca admite QAT mediante la modificación del grafo computacional para incorporar operaciones de cuantización durante el entrenamiento. En particular, se agregar un nodo de \textit{fake quantization} después de cada operación que deba cuantizarse. De este modo se simula el efecto de la cuantización en los pesos y activaciones durante el entrenamiento, similar al \textit{forward pass} desarrollado en la \reffig{single_layer_training_quantizer}.
...
Por ejemplo, en la multiplicación de dos tensores \texttt{a} y \texttt{b} del fragmento de código \ref{code:mul}, la función \texttt{mul} define la operación de multiplicación y \texttt{TensorFlow} calcula automáticamente los gradientes correspondientes. En este caso, el gradiente calculado se expresa en \eqref{eq:custom_gradient_grads_normal} .

\begin{equation}
\label{eq:custom_gradient_grads_normal}
...
El decorador \cite{pep318} \texttt{@tf.custom\_gradient} \cite{tensorflow_custom_gradient} es el que permite este flujo de trabajo. Al aplicarlo sobre una función de N argumentos: \texttt{arg1}, \texttt{arg2}, $\ldots$, \texttt{argN}, la función debe devolver una tupla, \texttt{y, grad}, donde \texttt{y} es el resultado de la operación y \texttt{grad} un \texttt{callable} \cite{python_callable} que devuelve la tupla \cite{python_datastructures} de gradientes con respecto a cada argumento, en el ejemplo del fragmento de código \ref{code:customgradient}, los gradientes son \texttt{da} y \texttt{db}.

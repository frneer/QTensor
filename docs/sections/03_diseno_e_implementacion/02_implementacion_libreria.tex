...
El objetivo de la librería es proporcionar una herramienta que permita realizar QAT con parámetros entrenables, que posibilite la experimentación con diferentes esquemas de cuantización, y que facilite la evaluación de los mismos.

Los requerimientos funcionales de la librería se definieron en base a los objetivos planteados en la sección \ref{sec:objetivos}. Los mismos se detallan en la tabla \ref{tab:rf_detailed}, donde se especifica el objetivo al que pertenecen, el numero del requisito y una descripción detallada del mismo.

\input{sections/03_diseno_e_implementacion/02_tab_requisitos_funcionales.tex}
...
\subsubsection{Desarrollo con contenedores}
Para asegurar la reproducibilidad del entorno de desarrollo y evitar problemas de compatibilidad entre diferentes versiones de librerías, se utilizó \texttt{Docker} \cite{docker} para crear un contenedor con todas las dependencias necesarias para el desarrollo y ejecución de la librería. El contenedor se define en un archivo \texttt{Dockerfile}, que especifica la imagen base, las dependencias a instalar y los comandos a ejecutar para configurar el entorno. A su vez se utilizó \texttt{docker-compose} \cite{docker_compose} para facilitar la gestión del contenedor y permitir la ejecución de comandos específicos dentro del mismo.

En particular, la librería se beneficia ampliamente de la utilización de una GPU para acelerar el entrenamiento de los modelos, por lo que se utilizó la imagen oficial de \texttt{Tensorflow} con soporte para GPU como base del contenedor utilizando nvidia container toolkit\cite{nvidia_container_toolkit}.

\subsubsection{Pipeline CI}
Para asegurar la calidad del código y la funcionalidad de la biblioteca, se implementó un pipeline de integración continua (CI) utilizando \texttt{GitHub Actions} \cite{github_actions}. Este pipeline se ejecuta automáticamente en \textit{pull requests} disparado por cualquier actualización en la rama de desarrollo. Y también es ejecutado al realizar un \textit{push} a la rama principal (\texttt{main}) para asegurar que la rama principal siempre esté en un estado funcional, y detectar condiciones de carrera al momento de fusionar ramas.

Este pipeline construye el contenedor de desarrollo y ejecuta las pruebas unitarias y de integración. Si alguna de estas etapas falla, el pipeline marca el \textit{pull request} o el \textit{push} como fallido, y bloquea la fusión de la rama hasta que se resuelvan los problemas.
...
\subsection{Descripción de la librería \texttt{QTensor}}

La librería \texttt{QTensor} fue diseñada como una capa de abstracción y extensión sobre \texttt{TFMOT}. Esto permite extender las funcionalidades existentes de \texttt{TFMOT} acercando la librería a los atributos de calidad definidos en la sección \ref{sec:atributos_calidad}. En particular, se se ha mejorado la ergonomía para definir esquemas de cuantización complejos, y permitir definir cuantizadores con parámetros entrenables y gradientes personalizados. También incluye un modulo para evaluar el rendimiento de los modelos cuantizados, y un modulo para gestionar flujos de trabajo de entrenamiento y cuantización.

En la \reffig{qtensor_modules} se muestra como es la interacción entre \texttt{QTensor}, \texttt{TFMOT}, \texttt{Keras} y \texttt{Tensorflow}.
...
El diseño de \texttt{QTensor} es modular, es decir, se compone de varios módulos, cada uno con una funcionalidad específica. En la \reffig{qtensor_overview} se muestra la estructura general de la librería y sus módulos principales.

\defaultdiagram{implementacion/qtensor_modules}{Estructura general de la librería QTensor.}{qtensor_overview}{1}
...
% \ TODO{Falta agregar activations al codigo...}
...
Cada paso queda definido principalmente por el modelo inicial del cual se parte (su entrada), una función que define el proceso a ejecutar, y los parámetros de esta función. A esto se le suma la semilla para la generación de números aleatorios, para asegurar que los resultados sean reproducibles.
...
Esta clase permite generar un hash único a partir de estos atributos, utilizando la función \texttt{to\_hash}.
...
Es importante destacar que para guardar y cargar los modelos deben ser serializables y deserializables. Para esto, se utiliz\'o la funcionalidad de Keras para guardar y cargar modelos en formato \texttt{keras}. Todo objeto desarrollado por la librería que forme parte del modelo como la configuracion (\texttt{GenerateConfig}) o los cuantizadores personalizados (\texttt{UniformQuantizer}, \texttt{FlexQuantizer}) implementan el método \texttt{get\_config} y el método de clase \texttt{from\_config}, para poder ser serializados y deserializados correctamente.

\subsubsection{Clase \texttt{Pipeline}}

La clase \texttt{Pipeline} es un orquestador de etapas. Esta clase permite definir un flujo de trabajo a partir de una lista de etapas, y ejecutar cada una de estas etapas en orden. La clase \texttt{Pipeline} se encarga de gestionar la ejecución de cada etapa, pasando los resultados intermedios entre las etapas, y guardando los resultados intermedios en disco.

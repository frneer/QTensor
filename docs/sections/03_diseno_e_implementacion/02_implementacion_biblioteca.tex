...
\toprule
\textbf{Req. ID} & \textbf{Requisito} & \textbf{Caracter\'{i}stica} & \textbf{Descripción} \\ \hline
RNF1.1 & Adecuación funcional & Pertinencia funciónal. & Capacidad del producto software para proporcionar un conjunto de funciónes que facilitan la consecución de tareas y objetivos de usuario especificados. \\ \hline
...
La biblioteca \texttt{QTensor} fue diseñada como una capa de abstracción y extensión sobre \texttt{TFMOT}. Esto permite extender las funcionalidades existentes de \texttt{TFMOT} para lograr obtener los requisitos funcionales descriptos en la sección \ref{sec:requerimientos_funcionales}. En la \reffig{qtensor_modules} se diagrama la interacción entre \texttt{QTensor}, \texttt{TFMOT}, \texttt{Keras} y \texttt{Tensorflow}.

\defaultdiagram{implementacion/tf_tfmot_qtensor_interaction}{Interacción entre QTensor, TFMOT, Keras y Tensorflow.}{qtensor_modules}{1}

El diseño de \texttt{QTensor} es modular, es decir, se compone de varios módulos, con funcionalidades específicas. Lo que facilita la incorporación de nuevas características, la mantenibilidad del código y la adaptación a diferentes casos de uso. Esta característica apoya el atributo de calidad RNF3.2 definido en la sección \ref{sec:atributos_calidad}.

En la \reffig{qtensor_overview} se muestra la estructura general de la biblioteca y sus módulos principales.

\defaultdiagram{implementacion/qtensor_modules}{Estructura general de la biblioteca QTensor.}{qtensor_overview}{1}

\begin{itemize}
    \item \texttt{quantizers}: módulo que contiene la implementación de los cuantizadores personalizados con parámetros entrenables y gradientes personalizados.
    \item \texttt{configs}: módulo que contiene abstracciones para definir configuraciones de cuantización como diccionarios de Python.
    \item \texttt{utils}: módulo que contiene la implementación de herramientas para evaluar el rendimiento de los modelos cuantizados.
    \item \texttt{stages}: módulo que contiene la implementación de las etapas del flujo de trabajo de entrenamiento y cuantización.
\end{itemize}
...
La interfaz \texttt{Quantizer} \cite{tensorflow_quantizer_api} define las operaciones matemáticas que transforman tensores durante la cuantización, la cual define dos métodos principales.
...
Para definir este comportamiento se implementaron restricciones en los parámetros entrenables utilizando la interfaz de keras \texttt{Constraint} \cite{tensorflow_keras_constraint}, la cual define una operación de restricción que se aplica a los tensores entrenables durante el entrenamiento. Se implementaron las siguientes restricciones personalizadas en el módulo \texttt{constraints}:
...
En el fragmento de código \ref{code:quantizer_constraints} se muestra la implementación de la restricción \texttt{PositiveConstraint}.
...
Por ejemplo, la configuracion de pesos es manejada por métodos descriptos en el fragmento de código \ref{code:generate_config}.
...
Para simplificar este proceso se diseño una interfaz de alto nivel, en el modulo \texttt{qmodel} que define las funciónes necesarias para cuantizar cualquier modelo definido en \texttt{TensorFlow} a partir de cualquier descripción de esquema de cuantización.
...
Para calcular la complejidad espacial de un modelo cuantizado, se implementaron las funciónes necesarias para calcular la complejidad espacial de modelos sin cuantizar y cuantizados, tanto para el caso de cuantización uniforme como para el esquema de cuantización flexible.

Para calcular la complejidad espacial de un modelo, se itera sobre las capas del modelo, y se calcula la complejidad espacial de cada capa, sumando los resultados. La complejidad espacial de una capa se calcula como la suma del tamaño de los pesos y las activaciones.

Como se desarrollo anteriormente, los esquemas de cuantización pueden contener cuantizadores de distintos tipos y parametros no cuantizados, con lo cual, para calcular la complejidad espacial de una capa, es necesario identificar el tipo de cuantizador utilizado para cada peso y activación, y aplicar la función de cálculo de complejidad espacial correspondiente. La implementación de esta lógica se muestra en el fragmento de código \ref{code:compute_model_space_complexity}.

\begin{lstlisting}[language=Python,label=code:compute_model_space_complexity,caption=Cálculo de la complejidad espacial de un modelo cuantizado.]
...
Donde la función \texttt{compute\_space\_complexity\_quantize} calcula la complejidad espacial de una capa cuantizada, determinando el tipo de cuantizador utilizado para cada parámtero, como se muestra en el fragmento de código \ref{code:compute_quantized_layer_space_complexity}.

\begin{lstlisting}[language=Python,label=code:compute_quantized_layer_space_complexity,caption=Cálculo de la complejidad espacial de una capa cuantizada.]
  def compute_space_complexity_quantize(qlayer: QuantizeWrapperV2) ->
...
Para una capa sin cuantizar, para obtener la complejidad espacial en bits se utiliza la función mostrada en el fragmento de código \ref{code:compute_space_complexity}.

\begin{lstlisting}[language=Python,label=code:compute_space_complexity,caption=Cálculo de la complejidad espacial de una capa sin cuantizar.]
...
Para el caso uniforme, el tamaño de cada peso, depende de la cantidad de bits utilizada en la cuantización. Por lo tanto, la complejidad espacial de los pesos cuantizados se calcula como se muestra en el fragmento de código \ref{code:complexity_uniform}.

\begin{lstlisting}[language=Python,label=code:complexity_uniform,caption=Cálculo de la complejidad espacial de un modelo cuantizado con el esquema uniforme.]
    weight_size = weight.shape.num_elements() * quantizer.bits
\end{lstlisting}

\subsubsection{Complejidad para el cuantizador flexible}

En el caso de la cuantización flexible, se considerara que la implementación utiliza la codificación de Huffman. Lo cual reduce el espacio necesario para almacenar los parámetros. Por lo tanto, la complejidad espacial de los parámetros del cuantizador se calculara como el valor nominal de la codificación de Huffman. Los detalles de la codificación de Huffman se encuentran en el apéndice \ref{appendix:huffman}.

\begin{lstlisting}[language=Python,label=code:complexity_flexible,caption=Cálculo de la complejidad espacial de un modelo cuantizado con el esquema flexible.]
...
A esto como se muestra en el fragmento de código \ref{code:compute_quantized_layer_space_complexity}, se le suma el espacio necesario para almacenar los niveles del cuantizador, pues se deben almacenar en alguna sección de la memoria del hardware donde se desplegará el modelo cuantizado.

\subsection{Implementación del modulo de flujo de trabajo (módulo \texttt{stages})}

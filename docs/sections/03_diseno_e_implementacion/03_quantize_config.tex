...
El fragmento \ref{code:tfmot_quantization_custom_layer} ilustra las complejidades que surgen al definir esquemas de cuantización personalizados en \texttt{TFMOT}. Cada vez que se desea modificar la configuración de cuantización, es necesario crear una nueva clase que implemente la interfaz \texttt{QuantizeConfig}. Por lo tanto, este enfoque dificulta la experimentación con diferentes esquemas de cuantización, ya que cada cambio requiere modificaciones significativas en el código.
...
En resumen, el enfoque nativo de \texttt{TFMOT} para definir esquemas de cuantización personalizados resulta repetitivo y poco ergonómico, lo que limita significativamente la capacidad de experimentación. Para comprender mejor estas limitaciones y proponer soluciones efectivas, a continuación se describe en detalle la interfaz \texttt{QuantizeConfig} y las mejoras implementadas en \texttt{QTensor} para facilitar la definición de esquemas de cuantización personalizados.
...
La interfaz \texttt{QuantizeConfig} constituye el componente central para describir cómo cuantizar una capa. Esta interfaz especifica los cuantizadores que deben aplicarse a los pesos, las activaciones y las salidas de la capa, como se muestra en la figura \ref{fig:tfmot_quantize_config}.

\defaultdiagram{implementacion/tfmot_quantize_config}{Interfaz \texttt{QuantizeConfig} de TFMOT.}{tfmot_quantize_config}{0.7}

Por ejemplo, la implementación de una configuración de cuantización personalizada \texttt{MyQuantizeConfig} para una capa \texttt{Dense} que define cuantizadores específicos para los pesos y \textit{biases}, y otro cuantizador para la activación, se realiza mediante la implementación de los métodos de la interfaz \texttt{QuantizeConfig}, como se muestra en el fragmento de código \ref{code:quantize_config}. En este caso se definen y utilizan dos cuantizadores (cuya interfaz será descrita en la sección \ref{sec:implementation_quantizers}): \texttt{MyQuantizer} y \texttt{SomeOtherQuantizer}.
...
Ante estas limitaciones, se introdujo la clase \texttt{GenerateConfig}, la cual permite la definición de esquemas de cuantización mediante diccionarios de Python, mejorando significativamente la ergonomía del proceso.
...
Desde el punto de vista técnico, la implementación de \texttt{GenerateConfig} adapta los métodos de la interfaz \texttt{QuantizeConfig} a esta nueva forma declarativa de definir las configuraciones. Para lograrlo, se aprovechó la capacidad de Python para inspeccionar los atributos de una clase en tiempo de ejecución mediante las funciones nativas \texttt{getattr} y \texttt{setattr}.
...
Como puede observarse, incluso con \texttt{GenerateConfig}, el código resulta extenso y requiere múltiples pasos manuales. Para abordar esta limitación, se diseñó el submódulo \texttt{qmodel}, que proporciona una interfaz de alto nivel para cuantizar cualquier modelo definido en \texttt{TensorFlow} a partir de esquemas de cuantización expresados como diccionarios de Python.

Con el uso de \texttt{qmodel}, el ejemplo del fragmento \ref{code:custom_cuantization_layout} se reduce considerablemente, como se muestra en el fragmento de código \ref{code:qmodel_usage}.
...
A continuación, se ejecuta un proceso de validación que verifica la existencia de todas las capas especificadas en el modelo. Posteriormente, se aplica la transformación final que reemplaza las capas anotadas por sus versiones cuantizadas. Adicionalmente, se gestiona el registro de objetos personalizados necesarios para garantizar la correcta serialización y deserialización del modelo cuantizado, lo que satisface los requisitos funcionales RF1.1, RF1.2 y RF1.4.
...
\defaultdiagram{implementacion/qtensor_pattern}{Patrón simplificado de uso de QTensor con el módulo \texttt{qmodel}.}{qtensor_pattern}{0.8}

En síntesis, la combinación de la clase \texttt{GenerateConfig} y el submódulo \texttt{qmodel} constituye una solución integral que transforma radicalmente la forma de definir y aplicar esquemas de cuantización personalizados en \texttt{TensorFlow}. Esta arquitectura ofrece tres ventajas fundamentales:

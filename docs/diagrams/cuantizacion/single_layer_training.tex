...
\draw[arrow] (phi.east) -- node[lbl_fwd, pos=0.75] {$a_{m}$} ($(phi)+(3,0)$);
...
\node[tensor, below left=of z] (theta) {$\theta$};
...
% % draw z gradient, positioned 0.75cm below
\node[layer_function, below=of z, yshift=-1.5cm] (dz) {$\nabla_z$};
...
\draw[arrow] ($(mult)+(3,0)$) -- (mult.east) node[lbl_bwd,pos=0.25] {$\frac{\partial \mathcal{L}}{\partial a_{m}}$};
\draw[arrow] (dphi) -- (mult);

% draw a circle with $\times in the midle of dz and dphi
\node[operation, below=of dz] (mult_dz) {$\times$};
\draw[arrow] (mult) -- (mult_dz);
\draw[arrow] (dz) -- (mult_dz);
% \draw[arrow] (mult_dz.west) -- (dz.east);

% --- MODIFIED: Changed from (-2,0) to (-3,0) ---
\draw[arrow] (mult_dz) -- node[lbl_bwd, pos=0.85] {$\frac{\partial \mathcal{L}}{\partial a_{m-1}}$} ($(mult_dz)+(-6,0)$);
...
% connect mult_dz
\draw[arrow] (mult_dz) -| (mult_eta) node[lbl_left, pos=0.75] {$\frac{\partial \mathcal{L}}{\partial \theta}$};
...
\begin{pgfonlayer}{background}
    \node [
        ...
        fit=(z) (phi) (theta) (mult_eta) (dz) (dphi) (mult) (mult_dz),
    ...
    \draw [gray, dashed, thin] ($(theta)+(-3,0)$) -- ($(theta)+(9.25,0)$);
\end{pgfonlayer}

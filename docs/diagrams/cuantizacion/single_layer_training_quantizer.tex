...
\node[layer_function, right=of phi] (qphi) {$q_\phi()$};
\draw[arrow] (phi) -- (qphi);
...
\node[layer_function, below left=of z] (qtheta) {$q_{\theta}()$};
...
\node[layer_function, below=of theta] (dqtheta) {$\nabla_{q_{\theta}}$};
...
\node[layer_function, right=of dphi] (dqphi) {$\nabla_{q_\phi}$};

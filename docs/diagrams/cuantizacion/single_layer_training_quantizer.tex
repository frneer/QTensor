...
\node[layer_function, right=of phi, pattern=north east lines, pattern color=gray!20] (qphi) {$q_\phi()$};
\draw[arrow] (phi) -- (qphi);
...
\node[layer_function, below left=of z, pattern=north east lines, pattern color=gray!20] (qtheta) {$q_{\theta}()$};
...
% Place gradient of q_theta just below theta (slightly left to avoid overlapping the update op)
\node[layer_function, below=of theta, pattern=north east lines, pattern color=gray!20] (dqtheta) {$\nabla_{q_{\theta}}$};
...
\node[layer_function, right=of dphi, pattern=north east lines, pattern color=gray!20] (dqphi) {$\nabla_{q_\phi}$};
